 ...
 * 1. Touch capability + coarse pointer + screen size (excludes touchscreen laptops)
 * 2. User-agent fallback for edge cases
 * 3. Special handling for iPadOS which spoofs "Macintosh" user-agent
...
const isMobileOrTablet = () => {
  ...
  // User-agent check for common mobile/tablet patterns
  const userAgentCheck = /Android|iPhone|iPad|iPod|webOS|BlackBerry|IEMobile|Opera Mini/i.test(
    navigator.userAgent
  );

  // Special check for iPadOS 13+ which spoofs as "Macintosh" but has touch
  // Real Macs with touchscreen don't report maxTouchPoints > 0
  const isIPadOS = navigator.userAgent.includes('Macintosh') &&
    navigator.maxTouchPoints > 1;

  return touchBasedCheck || userAgentCheck || isIPadOS;
};
